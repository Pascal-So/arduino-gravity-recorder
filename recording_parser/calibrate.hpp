...
Eigen::Vector3f get_x_axis(std::vector<Eigen::Vector3f> const& photo_gravity) {
	...
	Eigen::Vector4f w = m.bdcSvd(Eigen::ComputeFullV).matrixV().col(4);
	w /= w.head<3>().norm();
	...
	if (centre_of_positive_sector.dot(vecs_on_plane[1]) < centre_of_positive_sector.dot(vecs_on_plane[0]))
		normal *= -1;
...
Eigen::Quaternionf calibrate(std::ifstream& recording_infile, unsigned x_session_index, unsigned y_session_index, int nr_skip_measurements) {
	unsigned session_index = 0;

	if (x_session_index == y_session_index) {
		throw std::invalid_argument("The indices for calibrating the x and y axes can't be equal.");
	}
	...
	while (read_next_entry(recording_infile, entry_type)) {
		...
		case log_entry_types::photo_event:
			...
			if (session_index == x_session_index || session_index == y_session_index) {
				Eigen::Vector3f g = Eigen::Vector3f::Zero();
				int nr_relevant_measurements = measurements.size() - nr_skip_measurements;
	...
	rotation_matrix << x_axis, y_axis, x_axis.dot(y_axis);

	return Eigen::Quaternionf(rotation_matrix.transpose());
}
